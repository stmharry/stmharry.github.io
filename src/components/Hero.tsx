...
export const Hero = ({ profile }: HeroProps) => {
  return (
    <header className="mt-2 w-full sm:mt-4">
      ...
      <p className="mt-5 max-w-3xl text-sm leading-relaxed text-(--ink-700) sm:text-base">{profile.summaryLead}</p>
      <p className="mt-2 max-w-3xl text-xs leading-relaxed text-(--ink-700) sm:text-sm">{profile.summaryNote}</p>
    </header>
  );
};
